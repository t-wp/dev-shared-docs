...
- exit sell・rebalance sell それぞれの発火を観測ログで確認済み
- risk budget が市場強度に応じて変化していることを観測ログで確認済み
- all-negative → JPY 待機の切り替えを少なくとも 1 回観測済み
- state.json の異常値（peak_jpy 膨張など）が再発していない
