...
- **現在の実務フェーズ**: Step 7 本番移行判断のための bar-close dry_run 観測フェーズ
- **完了済みの土台整備**: cadence 修正、Dashboard Logs 整理、LogLayout 移行、2-slot 比較基盤の設計・実装・起動確認
- **2-slot 比較基盤**は champion / challenger の最小比較構成として成立確認済み
  - manifest 差分確認済み
  - /api/slots 確認済み
  - slot-2 cold-start の read-only 失敗モード確認済み
  - shared DB 作成後の自動復帰確認済み
- **現在の観測 interval**: 1hour（暫定 champion。replay で利益足と確認済み）

---

## 今フェーズの目的

- bar-close 観測の初期データを整理する（現在は 1hour で観測中）
...
- **現在の interval**: 1hour（暫定 champion。10min/30min は replay で全パターン損失）
- **interval 比較の扱い**: 同一 experiment 内では混在させない。interval 変更は別 experiment として切り出す
- **replay 側の主要改修**: 実施済み
- **本番初期 `param_set` 候補**: replay では primary 候補あり
- **2-slot 比較基盤**: 最小構成で成立確認済み
- **ローソク足データ欠損 WARN**:
  - 4/17〜4/19 の定常運用では再発 0 件
...
- bar-close dry_run 観測継続: https://github.com/t-wp/crypt-bot/issues/3
...
- AI half-day summary 設計・実装: https://github.com/t-wp/crypt-bot/issues/26
