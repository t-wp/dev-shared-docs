...
### deadband シミュレーション結果（2026-04-17）

現行ログのrebalance行（207件）から `超過率 = (current - target) / target` を抽出し、deadbandを変えた場合の残存件数をシミュレーション。

**超過率の分布:**
- 中央値: 17.5%, 25%ile: 9.8%, 75%ile: 33.3%
- 5%以下はわずか5件（target=0の完全撤退が大半）

**rebalance件数への効果:**

| deadband | 残存件数 | 除外率 | 残存代金 |
|---|---|---|---|
| 5%（現行） | 202 | 2% | ¥610,705 |
| 8% | 177 | 14% | ¥583,133 |
| 10% | 153 | 26% | ¥546,572 |
...
**全体売買指標への効果（buy+exit+rebalance合算）:**

| deadband | 件/週 | 片側TO/週 | ゲート判定 |
|---|---|---|---|
| 5%（現行） | 2,293 | 5,966% | **全NG** |
| 10% | 2,094 | 5,833% | **全NG** |
| 15% | 1,951 | 5,692% | **全NG** |
| 20% | 1,861 | 5,593% | **全NG** |

**結論: deadband拡大だけでは不十分。** rebalanceは全体の36%にすぎず、**buy=311件（55%、サイクルの81%で発火）が最大のドライバー**。10min足×6銘柄×トレンド継続シグナルの組み合わせが高頻度BUYの根本原因。

**必要な追加対策候補:**
1. **足の長期化**（10min → 1hour）: サイクル数が1/6になり、買い頻度が構造的に減少
2. **BUY cooldown**（同一銘柄で直近N本以内に約定済みなら再BUYしない）
3. **deadband + rebalance_min_jpy の併用強化**（小額rebalanceの完全排除）

### Portfolio Replay 比較結果（2026-04-17）

`portfolio_replay.py` で SQLite 保存済み OHLC を順送りし、bot.py のポートフォリオロジックをオフライン再現。
deadband シミュレーション（ログ後加工・近似）と異なり、**状態遷移を正確に再計算**している。

**データ:** 全6銘柄共通期間 3.3日（04-14 06:00 ~ 04-17 13:10）、初期資産 ¥100,000

#### Phase 1: interval × cooldown（deadband=0.05 固定）

| パラメータ | bars | 取引 | BUY | EXIT | REBAL | 純損益 | 手数料 | 最終資産 | maxDD% | 週取引 | 週TO% |
|---|---|---|---|---|---|---|---|---|---|---|---|
| 10min/cd0 | 267 | 106 | 45 | 13 | 48 | -573 | 214 | 99,427 | 0.7% | 225 | 912% |
| 10min/cd3 | 267 | 182 | 67 | 24 | 91 | -1,052 | 412 | 98,948 | 1.3% | 386 | 1,760% |
| 10min/cd6 | 267 | 102 | 35 | 18 | 49 | -1,050 | 295 | 98,950 | 1.3% | 216 | 1,257% |
| 30min/cd0 | 123 | 107 | 46 | 11 | 50 | -1,419 | 221 | 98,581 | 2.2% | 228 | 949% |
| 30min/cd3 | 123 | 115 | 41 | 13 | 61 | -912 | 285 | 99,088 | 2.1% | 245 | 1,217% |
| 30min/cd6 | 123 | 97 | 30 | 13 | 54 | -862 | 267 | 99,138 | 2.0% | 206 | 1,142% |
| **1hour/cd0** | 52 | 135 | 79 | 3 | 53 | **+3,213** | 148 | **103,213** | 1.7% | 287 | 618% |
| 1hour/cd3 | 52 | 97 | 43 | 3 | 51 | +2,810 | 136 | 102,810 | 1.7% | 206 | 571% |
| 1hour/cd6 | 52 | 80 | 32 | 3 | 45 | +2,772 | 130 | 102,772 | 1.6% | 170 | 545% |

**Gate判定:** 10min=2/4, 30min=2〜3/4, **1hour=3/4**（残るNGは週TOのみ）

#### Phase 2: 1hour × cooldown × deadband 全組み合わせ

| パラメータ | 取引 | BUY | REBAL | 純損益 | 手数料 | 最終資産 | maxDD% | 週取引 | 週TO% | Gate |
|---|---|---|---|---|---|---|---|---|---|---|
| cd0/db0.05 | 135 | 79 | 53 | +3,213 | 148 | 103,213 | 1.7% | 287 | 618% | 3/4 |
| cd0/db0.10 | 106 | 70 | 33 | +3,312 | 138 | 103,312 | 1.8% | 225 | 576% | 3/4 |
| cd0/db0.15 | 90 | 62 | 25 | +3,381 | 138 | 103,381 | 1.8% | 191 | 577% | 3/4 |
| **cd0/db0.20** | **78** | 56 | 19 | **+3,343** | 127 | **103,343** | 1.8% | **166** | **530%** | **3/4** |
| cd3/db0.05 | 97 | 43 | 51 | +2,810 | 136 | 102,810 | 1.7% | 206 | 571% | 3/4 |
| cd3/db0.10 | 79 | 45 | 31 | +2,885 | 132 | 102,885 | 1.7% | 168 | 555% | 3/4 |
| cd3/db0.15 | 71 | 42 | 26 | +2,952 | 131 | 102,952 | 1.7% | 151 | 550% | 3/4 |
| cd3/db0.20 | 63 | 40 | 20 | +2,873 | 123 | 102,873 | 1.7% | 134 | 517% | 3/4 |
| cd6/db0.05 | 80 | 32 | 45 | +2,772 | 130 | 102,772 | 1.6% | 170 | 545% | 3/4 |
| cd6/db0.10 | 65 | 31 | 31 | +2,844 | 126 | 102,844 | 1.7% | 138 | 528% | 3/4 |
| cd6/db0.15 | 56 | 31 | 22 | +2,849 | 124 | 102,849 | 1.7% | 119 | 519% | 3/4 |
| cd6/db0.20 | 53 | 31 | 19 | +2,725 | 120 | 102,725 | 1.7% | 113 | 505% | 3/4 |

#### 主要な発見

1. **interval が最大の効果**: 10min/30min は全パターン損失、1hour のみ全パターン利益。interval 変更が PnL に対する最大レバー
2. **cooldown の逆効果**: 10min/cd3 は cd0 より取引増（182 vs 106）。cooldown で BUY を遅延させると、後続のリバランスチャーンが増幅される。1hour では効果方向は正しいが PnL は低下（cd0=+3,213 > cd6=+2,772）
3. **deadband は単調に効く**: rebalance 件数が 53→19 に減少し、手数料も低下。PnL は db0.15 がピーク（+3,381）
4. **最残課題は週TO**: 全パターン 500%超。Gate 4/4 達成にはさらなる対策が必要（BUY条件追加 or 銘柄数削減）
5. **データ制約**: 3.3日（1hour=52bars）のみ。結論は暫定。2週間以上のデータ蓄積後に再検証が必要
